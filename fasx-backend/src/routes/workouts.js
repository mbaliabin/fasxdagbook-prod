...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (!userId) return res.status(401).json({ error: 'Пользователь не аутентифицирован' })

    const {
      name, type, date, duration, distance,
      comment, effort, feeling, intensityZones,
    } = req.body
    ...
    const validTypes = [
      'Running', 'XC_Skiing_Skate', 'XC_Skiing_Classic',
      'RollerSki_Skate', 'RollerSki_Classic', 'StrengthTraining',
      'Swimming', 'Bike', 'Other',
    ]
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!existingWorkout || existingWorkout.userId !== userId) {
      return res.status(404).json({ error: 'Тренировка не найдена или недоступна' })
    }
