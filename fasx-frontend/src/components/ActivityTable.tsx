...
import { X, Calendar, ChevronRight, TrendingUp, Timer, Inbox } from 'lucide-react';
...
/** * Исправленная логика:
 * Сначала проверяем на роллеры и имитацию,
 * чтобы они не "проваливались" в общее условие лыж.
 */
...
export default function ActivityTable({ workouts }: Props) {
  ...
  return (
    <div className="w-full font-sans text-white">
      {/* 1. ЗАГОЛОВОК (Без иконки активности) */}
      <div className="flex justify-between items-center mb-4 px-1">
        <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Аналитика тренировок</h2>
      </div>
          ...
          <div className="p-2 space-y-1">
            {activityRows.map((row, idx) => (
              <button
                ...
                className="w-full flex items-center justify-between py-4 px-5 rounded-2xl hover:bg-white/[0.03] transition-all group active:scale-[0.99]"
              >
                <div className="flex flex-col gap-2.5 flex-1 text-left min-w-0 pr-6">
                  <span className="text-[11px] font-bold text-gray-200 group-hover:text-blue-400 transition-colors truncate uppercase tracking-wide">
                    {row.type}
                  </span>
              ...
              >
                <Dialog.Panel className="relative w-full max-w-2xl transform overflow-hidden rounded-[32px] bg-[#0a0a0b] border border-gray-800/60 p-0 shadow-2xl transition-all">
                  ...
                  <div className="p-8 pb-4 flex justify-between items-start">
                    <div>
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 mb-1">{selectedType}</h3>
                  ...
                  <div className="grid grid-cols-3 gap-3 px-8 py-6">
                    <div className="bg-[#0f0f11] border border-gray-800/40 p-5 rounded-2xl flex flex-col items-center text-center">
                      <Timer size={18} className="text-gray-500 mb-3" />
                    ...
                    <div className="bg-[#0f0f11] border border-gray-800/40 p-5 rounded-2xl flex flex-col items-center text-center">
                      <TrendingUp size={18} className="text-gray-500 mb-3" />
                    ...
                    <div className="bg-[#0f0f11] border border-gray-800/40 p-5 rounded-2xl flex flex-col items-center text-center">
                      <div className="text-gray-500 mb-3 font-black text-[14px]">Ø</div>
