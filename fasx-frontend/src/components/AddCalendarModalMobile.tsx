...
import { X, Check, Ruler, ChevronDown } from "lucide-react";
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-hide pb-32">
                ...
                {(type !== "StrengthTraining" && type !== "Other" && type !== "") && (
                  <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2">
                    ...
                    <input type="number" step={0.01} value={distance} onChange={(e) => setDistance(e.target.value)} placeholder="0.00" className="w-full p-3 rounded-xl bg-[#2a2a2d] border border-blue-900/30 text-white font-bold text-lg outline-none no-spinner" />
                  </div>
                )}
